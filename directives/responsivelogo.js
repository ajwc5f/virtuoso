(function() {
	...
	module.directive('responsiveLogo', function() {
		return {
			restrict: 'E',
			scope: {
				imagesData: '=ngModel'
			...
			compile: function (tElem, tAttrs) {
				var linkFunction = function($scope, element, attributes) {
                	...
                	var cover = images[0].media[4].url;
									for (var i=5; i<images.length; i++) {
										if (images[i].media[4].width == targetWidth && images[i].media[4].height == targetHeight) {
											cover = images[i].media[4].url;
										}
									}
